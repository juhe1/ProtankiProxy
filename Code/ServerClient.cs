...
namespace ProtankiProxy
{
	...
	public class ServerClient : TankiTcpClient
	{
		...
		public ServerClient(
			...
			TankiTcpClientHandler clientHandler,
		...
		{
			Log.Information(
				"Connected to ProTanki server {Server}:{Port}",
				_serverAddress,
				_serverPort
			);
		...
		protected override Task OnPacketReceivedAsync(Packet packet)
		{
			...
			{
				var keys = ((ActivateProtection)packet).Keys;
				if (!(keys is null))
				{
			...
			var args = new PacketEventArgs(
				...
				clientHandler.GetClientEndPoint()
			);
		...
		{
			Log.Information(
				"Disconnected from server {Server}:{Port}",
				_serverAddress,
				_serverPort
			);
			return Task.CompletedTask;
		...
		protected override Task OnPacketUnwrapFailureAsync(
			Type packetType,
			int packetId,
			Exception exception
		)
		{
			Log.Error(
				exception,
				"Failed to unwrap packet of type {PacketType} with ID {PacketId}",
				packetType.Name,
				packetId
			);
			return Task.CompletedTask;
